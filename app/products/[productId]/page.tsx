...
const Product = ({ params }: { params: { productId: string } }) => {
  ...
  return (
    <section className='p-6 min-h-screen bg-emerald-50 relative'>
          ...
          <div className='grid grid-cols-[70%_30%]'>
            {allReview.length > 0 ?
              <div className='flex flex-col gap-12'>
                ...
                {reviewsError && <p className="text-red-500">Error loading product: {reviewsError.message}</p>}
            ...
            <div className='ml-auto'>
              {localUser?.id &&
                <button className='bg-emerald-500 text-white font-bold px-4 py-2 rounded-lg hover:bg-emerald-600 transition-colors' onClick={() => setToggleAddReview(!toggleAddReview)}>Add a review</button>
              }
            </div>
            </div>
        </div>
      ...
      {toggleAddReview &&
        <div className='absolute w-full h-full inset-0 bg-neutral-950/30 backdrop-blur-md' onClick={() => setToggleAddReview(!toggleAddReview)}>
            ...
            <div className='bg-white max-w-2xl mx-auto mt-8 rounded-xl p-16 py-12' onClick={(e) => e.stopPropagation()}>
              ...
              <form className='flex flex-col gap-4' onSubmit={handleSubmit}>
                ...
                <div className='flex gap-4 ml-auto'>
                  <button className='bg-red-400 hover:bg-red-500 text-white font-bold text-lg px-6 py-1 transition-colors' type='button' disabled={submitLoading} onClick={(e) => { e.preventDefault(); setFormReviewData({ title: '', rating: 0, comment: '' }); setToggleAddReview(!toggleAddReview); }}>Clear & Close</button>
